...
export interface SchemaConfigOptions<T extends readonly Schema[]> {
    schemas?: T,
    onMissingSchema?: MissingSchemaStrategy
}

export type SchemaMap<T extends readonly Schema[]> = {[K in T[number] as K['sheetName']] : K}
...
class SchemaConfig<T extends readonly Schema[]>{
    ...
    private makeSchemaMap(schemas:T){
        const schemaMap = schemas.reduce((schemaDefinition, schema) => {
            ...
            schemaDefinition[key] = schema as SchemaMap<T>[typeof key]
